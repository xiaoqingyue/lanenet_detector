...
class CNNBaseModel(object):
    ...
    def conv2d(inputdata, out_channel, kernel_size, padding='SAME',
        ...
        with tf.variable_scope(name):
            ...
            if w_init is None:
                w_init = tf.contrib.layers.variance_scaling_initializer()   # 激活函数relu 权重初始化
            if b_init is None:
                b_init = tf.constant_initializer()  # 常量初始化函数
            ...
            ret = tf.identity(tf.nn.bias_add(conv, b, data_format=data_format)
                              if use_bias else conv, name=name)    # y = tf.identity(x) 是一个op操作表示将x的值赋予y
    ...
    def layergn(inputdata, name, group_size=32, esp=1e-5):
        ...
        with tf.variable_scope(name):
            ...
            # 每个通道的gamma和beta
            gamma = tf.Variable(tf. tf.equaconstant(1.0, shape=[c]), dtype=tf.float32, name='gamma')
