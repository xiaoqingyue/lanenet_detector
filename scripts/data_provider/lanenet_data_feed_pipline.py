...
sys.path.append('/home/zhy/Documents/Perception/Lanelinedetection/DNN/LaneNet/lanenet-lane-detection')
sys.path.append('/home/zhy/Documents/Perception/Lanelinedetection/DNN/LaneNet/lanenet-lane-detection/config')
sys.path.append('/home/zhy/Documents/Perception/Lanelinedetection/DNN/LaneNet/lanenet-lane-detection/lanenet_model')
sys.path.append('/home/zhy/Documents/Perception/Lanelinedetection/DNN/LaneNet/lanenet-lane-detection/tools')
...
def init_args():
    ...
    parser.add_argument('--dataset_dir', type=str, help='The source nsfw data dir path',
                        default='/home/zhy/Documents/Perception/Lanelinedetection/DNN/LaneNet/lanenet-lane-detection'
                                '/data/tusimple_dataset/tuSimple/training/')
    parser.add_argument('--tfrecords_dir', type=str, help='The dir path to save converted tfrecords',
                        default='/home/zhy/Documents/Perception/Lanelinedetection/DNN/LaneNet/lanenet-lane-detection'
                                '/data/tusimple_dataset/tuSimple/training/tfrecords')
...
class LaneNetDataProducer(object):
    ...
    def __init__(self, dataset_dir):
        """
        # 检查是否包含所有文件，没有就生成所需文件 gt_image gt_binary_iamge  gt_instance_image
    ...
    def generate_tfrecords(self, save_dir, step_size=10000):
        ...
        # start generating training example tfrecords    train 的tfrecords产生
        log.info('Start generating training example tfrecords')

        # collecting train images paths info    将gt_image gt_binary gt_instance 的全部路径返回和保存
        ...
        for index, example_gt_paths in enumerate(train_example_gt_paths):  # enumerate遍历数据同时列出数据下标和数据
            tf_io_pipline_tools.write_example_tfrecords(  # 获得tfrecord
                example_gt_paths,
        ...
        # start generating validation example tfrecords  val 的tfrecords产生
        ...
        # generate test example tfrecords    test 的tfrecords产生
    ...
    def _generate_training_example_index_file(self):
        ...
        def _split_training_examples(_example_info):
            ...
            _train_example_nums = len(_train_example_info)
            _val_example_nums = len(_val_example_info)
            _test_example_nums = len(_test_example_info)
...
class LaneNetDataFeeder(object):
    ...
    def inputs(self, batch_size, num_epochs):
        """
        dataset feed pipline input  self._frontend_net_map = {
        ...
        :return: A tuple (images, labels), where:
                    ...
                    * labels is an int32 tensor with shape [batch_size] with the true label,
                      a number in the range [0, CLASS_NUMS).   # 输出  图像（batch_size, H , W ,C）
                                                               # 输出  标签 batch_size 大小的张量 类型是 int32
        """
        if not num_epochs:
            num_epochs = None
        ...
        random.shuffle(tfrecords_file_paths)  # 打乱顺序
        ...
        with tf.name_scope('input_tensor'):
            ...
            dataset = tf.data.TFRecordDataset(tfrecords_file_paths)  # 用dataset读取TFRecords文件
            ...
            if self._dataset_flags != 'test':
                dataset = dataset.map(map_func=tf_io_pipline_tools.augment_for_train,  # 图像翻转 图像剪裁
                                      num_parallel_calls=CFG.TRAIN.CPU_MULTI_PROCESS_NUMS)
            else:
                dataset = dataset.map(map_func=tf_io_pipline_tools.augment_for_test,
                                      num_parallel_calls=CFG.TRAIN.CPU_MULTI_PROCESS_NUMS)
            dataset = dataset.map(map_func=tf_io_pipline_tools.normalize,  # 图像归一化
                                  num_parallel_calls=CFG.TRAIN.CPU_MULTI_PROCESS_NUMS)

            # The shuffle transformation uses a finite-sized buffer to shuffle elements in memory.
            # The parameter is the number of elements in the buffer.
            # 该参数是缓冲区中元素的数量。
            # For completely uniform shuffling, set the parameter to be the same as the
            # number of elements in the dataset.   要完全均匀地进行混洗，请将参数设置为与数据集中的元素数相同。
            if self._dataset_flags != 'test':
                dataset = dataset.shuffle(buffer_size=1000)  # shuffle是按顺序将数据放入buffer里面的
                # repeat num epochs
                dataset = dataset.repeat()
            # 每次从buffer中抽取batch_size个样本
            dataset = dataset.batch(batch_size, drop_remainder=True)

            iterator = dataset.make_one_shot_iterator()  # 构造获取数据的迭代器
            # 每次从迭代器中获取一批数据
