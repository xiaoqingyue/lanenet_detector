...
def bytes_feature(value):
    """
DNN/LaneNet/lanenet-lane-detection/data/tusimple_dataset/tuSimple/training/gt_image/3039.png', '/home/zhy/Documents/Perception/Lanelinedetection/DNN/LaneNet/lanenet-lane-detection/data/tusimple_dataset/tuSimple/training/gt_image
    :param value:
    :return:
    """
...
def normalize(gt_image, gt_binary_image, gt_instance_image):
    """
    Normalize the image data by substracting the imagenet mean value 通过减去image net平均值对图像数据进行归一化
    :param gt_image:
    :param gt_binary_image:
    :param gt_instance_image:
    :return:
...
def random_crop_batch_images(gt_image, gt_binary_image, gt_instance_image, cropped_size):
    """
    Random crop image batch data for training  随机裁剪图像
    :param gt_image:
    :param gt_binary_image:
    :param gt_instance_image:
    :param cropped_size:
...
def random_horizon_flip_batch_images(gt_image, gt_binary_image, gt_instance_image):
    """
    Random horizon flip image batch data for training   随机翻转增强
    :param gt_image:
    :param gt_binary_image:
    :param gt_instance_image:
    :return:
    """
    concat_images = tf.concat([gt_image, gt_binary_image, gt_instance_image], axis=-1)  # 矩阵拼接
