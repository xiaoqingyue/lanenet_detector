...
class _LaneNetCluster(object):
    ...
    def _embedding_feats_dbscan_cluster(embedding_image_feats):
        ...
        try:
            features = StandardScaler().fit_transform(embedding_image_feats)  # StandardScaler作用：去均值和方差归一化,且是针对每一个特征维度来做的，而不是针对样本。
            db.fit(features)
...
class LaneNetPostProcessor(object):
    ...
    def __init__(self, ipm_remap_file_path='/home/zhy/Documents/lannetROS/src/lane_detector/scripts/model/jinan_ipm_remap.yml'):
        """
    ...
    def _load_remap_matrix(self):
        ...
        :return:
        """
        fs = cv2.FileStorage(self._ipm_remap_file_path, cv2.FILE_STORAGE_READ)  # 加载YMl文件数据
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        # lane line fit
        fit_params = []
        src_lane_pts = []  # lane pts every single lane 每条车道的点数
        for lane_index, coords in enumerate(lane_coords):  # lane_coords 每条车道线的坐标点
            ...
            plot_y = np.linspace(10, ipm_image_height, ipm_image_height - 10)  # 创建等差数列
            ...
            for index in range(0, plot_y.shape[0], 5):
                src_x = self._remap_to_ipm_x[
                    int(plot_y[index]), int(np.clip(fit_x[index], 0, ipm_image_width - 1))]  # clip 将数组中的元素限制在a_min, a_max之间，大于a_max的就使得它等于 a_max，小于a_min,的就使得它等于a_min.
                if src_x <= 0:
                    continue
        ...
        # tusimple test data sample point along y axis every 10 pixels  ＃tusimple测试数据样本每10个像素沿y轴采样一次
        ...
        for index, single_lane_pts in enumerate(src_lane_pts):
            ...
            if data_source == 'tusimple':
                ...
                raise ValueError('Wrong data source now only support tusimple and beec_ccd')
            step = int(math.floor((end_plot_y - start_plot_y) / 10))  # 向下取整
