...
class LaneNet(cnn_basenet.CNNBaseModel):
    ...
    def inference(self, input_tensor, name):
        ...
        with tf.variable_scope(name_or_scope=name, reuse=self._reuse):
            # first extract image features  编码+解码 test 卷积和反卷积
            ...
            # second apply backend process  预测结果
    ...
    def compute_loss(self, input_tensor, binary_label, instance_label, name):
        ...
        :return:
        """
        with tf.variable_scope(name_or_scope=name, reuse=self._reuse):    # 变量的作用域管理器；
            # first extract image features  提取图像特征 ----前向传播  建立模型   (编码conv+解码deconv)
            extract_feats_result = self._frontend.build_model(
                input_tensor=input_tensor,
                name='{:s}_frontend'.format(self._net_flag),
                reuse=self._reuse
            ...
            # second apply backend process    反向传播  计算损失函数
