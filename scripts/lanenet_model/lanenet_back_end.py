...
class LaneNetBackEnd(cnn_basenet.CNNBaseModel):
    ...
    def compute_loss(self, binary_seg_logits, binary_label,
        ...
        with tf.variable_scope(name_or_scope=name, reuse=reuse):
            # calculate class weighted binary seg loss  计算binary的损失函数
            with tf.variable_scope(name_or_scope='binary_seg'):
                binary_label_onehot = tf.one_hot(
                    ...
                    axis=-1
                )  # 用于分类任务中作为最后的FC层的输出  # 0/1 矩阵， 256x512x2

                binary_label_plain = tf.reshape(   # 将tensor变换成参数shape形式
                    binary_label,
                    shape=[binary_label.get_shape().as_list()[0] *
                           binary_label.get_shape().as_list()[1] *
                           binary_label.get_shape().as_list()[2] *
                           binary_label.get_shape().as_list()[3]])
                unique_labels, unique_id, counts = tf.unique_with_counts(binary_label_plain)  # 在一维张量中找到唯一的元素
                counts = tf.cast(counts, tf.float32)     # 强制类型转换
                ...
                binary_segmenatation_loss = self._compute_class_weighted_cross_entropy_loss(  # 交叉熵
                    onehot_labels=binary_label_onehot,  # binary_label
                    logits=binary_seg_logits,  # 计算出的输入解码
                    classes_weights=inverse_weights
                )

            # calculate class weighted instance seg loss      计算instance损失函数
            with tf.variable_scope(name_or_scope='instance_seg'):

                pix_bn = self.layerbn(
                    inputdata=instance_seg_logits, is_training=self._is_training, name='pix_bn')  # 归一化
                pix_relu = self.relu(inputdata=pix_bn, name='pix_relu')
                pix_embedding = self.conv2d(
                    inputdata=pix_relu,
                    out_channel=CFG.TRAIN.EMBEDDING_FEATS_DIMS,  # 嵌入特征
                    kernel_size=1,
                    use_bias=False,
            ...
            ret = {
                ...
                'binary_seg_logits': binary_seg_logits,  # binary 输出
                'instance_seg_logits': pix_embedding,    # instance 输出
